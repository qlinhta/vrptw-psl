...
from utils import calc_distance, check_time, start_times, check_sequence
...
def graph(tours: List[List[int]], x: List[float], y: List[float], dist_matrix: np.ndarray, file_name: str,
    ...
    ax.set_title(
        f"Sweep Alg wt Nearest Neighbor: {file_name} wt {cust_size}, distance: {calc_distance(tours, dist_matrix):.2f}, vehicles: {len(tours)}")
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    os.makedirs('./figures/sweep', exist_ok=True)
    fig.savefig(f'./figures/sweep/sweep_{file_name}_{cust_size}.pdf', format='pdf')
    plt.close(fig)
...
def is_feasible_tour(tour, dist_matrix, servicetime, readytime, duetime, demand, capacity):
    total_demand = sum(demand[node] for node in tour)
    ...
    for i in range(len(tour) - 1):
        ...
        if current_time < readytime[next_node]:
            current_time = readytime[next_node]
        if current_time > duetime[next_node]:
            return False
        current_time += servicetime[next_node]

    return True


def sweep_nearest_neighbor(cust_size: int, dist_matrix: np.ndarray, servicetime: List[float], readytime: List[float],
                           duetime: List[float], demand: List[int], capacity: int, xcoor: List[float],
                           ycoor: List[float]) -> List[List[int]]:
    depot_x, depot_y = xcoor[0], ycoor[0]
    angles = [(i, calculate_polar_angle(xcoor[i] - depot_x, ycoor[i] - depot_y)) for i in range(1, cust_size + 1)]
    angles_sorted = sorted(angles, key=lambda x: x[1])

    unrouted_customers = [x[0] for x in angles_sorted]
    initial_tours = []
        ...
        while unrouted_customers:
            ...
            for customer in unrouted_customers:
                if current_load + demand[customer] <= capacity:
                    distance = dist_matrix[current_tour[-1]][customer]
                    tentative_tour = current_tour + [customer, 0]
                    if distance < nearest_distance and is_feasible_tour(tentative_tour, dist_matrix, servicetime,
                                                                        readytime, duetime, demand, capacity):
...
def solve_initial_solution(cust_size: int, dist_matrix: np.ndarray, servicetime: List[float], readytime: List[float],
                           duetime: List[float], demand: List[int], capacity: int, xcoor: List[float],
                           ycoor: List[float]) -> List[List[int]]:
    return sweep_nearest_neighbor(cust_size, dist_matrix, servicetime, readytime, duetime, demand, capacity, xcoor,
                                  ycoor)
...
def run_vns(file_names: List[str], cust_size: int, iterations: int = 10):
    ...
    print(summary_table)
    save_results_to_csv(results, f'./results/sweep/sweep_heuristic_results_{cust_size}.csv')
...
def main():
    ...
    run_vns(args.files, args.customers, iterations=args.iterations)
