...
def solve_vrptw(file: str, cust_size: int, large_constant: int):
    ...
    x = {(i, j, k): solver.BoolVar(f'x[{i},{j},{k}]') for i, j, k in X}
    s = {(i, k): solver.NumVar(0.0, solver.infinity(), f's[{i},{k}]') for i, k in S}
    ...
    if status == pywraplp.Solver.OPTIMAL:
        ...
        objective_value = round(solver.Objective().Value(), 2)
        logger.info(colorful_log(
            f'File: {file} with {cust_size} customers, {no_vehicles} vehicles, feasible solution value: {objective_value}, elapsed time: {elapsed_time}, running time: {running_time}',
            'yellow'))
        logger.info(colorful_log(f'Finished processing file: {file}', 'blue'))
        return file, cust_size, no_vehicles, objective_value, elapsed_time, running_time
    else:
