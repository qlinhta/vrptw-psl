...
def graph(tours: List[List[int]], x: List[float], y: List[float], dist_matrix: np.ndarray, file_name: str,
    ...
    ax.set_title(
        f"Clarke Wright Savings: {file_name} wt {cust_size}, distance: {calc_distance(tours, dist_matrix):.2f}, vehicles: {len(tours)}")
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    os.makedirs('./figures/cws/', exist_ok=True)
    fig.savefig(f'./figures/cws/cws_{file_name}_{cust_size}.pdf', format='pdf')
    plt.close(fig)
...
def solve_initial_solution(cust_size: int, dist_matrix: np.ndarray, servicetime: List[float], readytime: List[float],
                           duetime: List[float], demand: List[int], capacity: int, mu: int) -> List[List[int]]:
    Sub_tour = []
    for i in range(cust_size + 1):
        sub = [0, i, 0]
        Sub_tour.append(sub)
    ...
    for i in range(1, sub_size + 1):
        for j in range(1, sub_size + 1):
            if i != j:
                saving = round(dist_matrix[i, 0] + dist_matrix[0, j] - mu * dist_matrix[i, j], 2)
    ...
    while len(saving_list_sorted) != 0:
        ...
        if a != b:
            ...
            time_check1 = check_time(new_sub1, dist_matrix, servicetime, readytime, duetime)
            time_check2 = check_time(new_sub2, dist_matrix, servicetime, readytime, duetime)
...
def run_vns(file_names: List[str], cust_size: int, mu: int, iterations: int = 10):
    ...
    print(summary_table)
    save_results_to_csv(results, f'./results/cws/cws_results_{cust_size}.csv')
...
def main():
    ...
    run_vns(args.files, args.customers, mu=1, iterations=args.iterations)
